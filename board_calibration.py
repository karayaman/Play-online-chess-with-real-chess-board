...
def get_config():
    ...
    return show_info, cap_index, cap_api, platform_name


def main():
    show_info, cap_index, cap_api, platform_name = get_config()
    ...
    if show_info:
        ...
        messagebox.showinfo(
            "Board Calibration",
            "Board calibration will start. It should detect corners of the chessboard"
            'almost immediately. If it does not, you should press key "q" to '
            "stop board calibration and change webcam/board position.",
        )
        if platform_name == "Darwin":
            root.destroy()
        ...
        if retval:
            if show_info:
                ...
                messagebox.showinfo(
                    ...
                    "Please check that corners of your chessboard are correctly detected. "
                    "The square covered by points (0,0), (0,1),(1,0) and (1,1) should be a8. "
                    "You can rotate the image by pressing key 'r' to adjust that. Press "
                    "key \"q\" to save detected chess board corners and finish board calibration.",
                )
                root.destroy()
            if (
    ...
    last_row = euclidean_distance(
        augmented_corners[7][1], augmented_corners[7][7])
    ...
    with pl.Path("constants.bin").open("wb") as outfile:
        pickle.dump(
...
def mark_corners(frame, augmented_corners, rotation_count):
    ...
    for i, row in enumerate(augmented_corners):
        for j, col in enumerate(row):
            if rotation_count == 0:
                ...
                index = f"{j},{8 - i}"
                corner = height - col[1], col[0]
            elif rotation_count == 2:
                index = f"{8 - i},{8-j}"
                corner = width - col[0], height - col[1]
            elif rotation_count == 3:
                index = f"{8 - j},{i}"
                corner = col[0], width - col[0]
