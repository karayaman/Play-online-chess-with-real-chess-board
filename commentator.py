...
class CommentatorThread(Thread):
    ...
    def run(self):
        ...
        while not self.game_state.board.is_game_over():
            ...
            if found_move and (
                (self.comment_me and is_my_turn)
                or (self.comment_opponent and (not is_my_turn))
            ):
                self.speech_thread.put_text(
                    self.language.comment(self.game_state.board, move)
                )
...
class GameState:
    ...
    def get_valid_move(
        ...
        if (
            ...
            if (
                self.board.peek() != chess.Move.from_uci("e1g1")
        ...
        if (
            ...
            if (
                self.board.peek() != chess.Move.from_uci("e1c1")
            ) and self.can_image_correspond_to_chessboard(
        ...
        if (
            ...
            if (
                self.board.peek() != chess.Move.from_uci("e8g8")
            ) and self.can_image_correspond_to_chessboard(
        ...
        if (
            ...
            if (
                self.board.peek() != chess.Move.from_uci("e8c8")
            ) and self.can_image_correspond_to_chessboard(
    ...
    def has_square_image_changed(self, old_square, new_square):
    ...
    def register_move_if_needed(self):
        ...
        if potential_starts:  # Fix for premove
            if len(self.registered_moves) < len(self.game_thread.played_moves):
                valid_move_UCI = self.game_thread.played_moves[
                    len(self.registered_moves)
                ]
                self.register_move(
                    valid_move_UCI, self.previous_chessboard_image)
                return True, valid_move_UCI
