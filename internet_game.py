...
class InternetGame:
    def __init__(self, use_template, start_delay, drag_drop):
        ...
        if use_template:
            ...
            (
                self.position,
                self.we_play_white,
            ) = chessboard_detection.auto_find_chessboard()
    ...
    def get_square_center(self, square_name):
        row, column = self.convert_square_name_to_row_column(
            square_name, self.we_play_white
        )
        position = self.position
        centerX = int(
            position.minX + (column + 0.5) *
            (position.maxX - position.minX) / 8
        )
        centerY = int(position.minY + (row + 0.5) *
                      (position.maxY - position.minY) / 8)
    ...
    def convert_row_column_to_square_name(
        self, row: int, column: int, is_white_on_bottom: bool
    ) -> str:
        if is_white_on_bottom is True:
            number = repr(8 - row)
