...
def log_process(process, finish_message):
    ...
    board = tk.Button(button_frame, text="Board Calibration",
                      command=board_calibration)
...
def start_game():
    ...
    if selected_voice != VOICE_OPTIONS[0]:
        ...
        for l, code in zip(languages, codes):
            if (
                (l in selected_voice)
                or (l.lower() in selected_voice)
                or (code in selected_voice)
            ):
                language = l
                break
...
voice_menu.config(width=max(len(option)
                  for option in VOICE_OPTIONS), anchor="w")
voice_menu.grid(column=1, row=0, sticky=tk.W)


def save_promotion(*_):
    with pl.Path("promotion.bin").open("wb") as outfile:
...
c1 = tk.Checkbutton(
    window, text="Make moves of opponent too.", variable=make_opponent)
c1.grid(row=4, column=0, sticky="W", columnspan=1)

c2 = tk.Checkbutton(
    window, text="Make moves by drag and drop.", variable=drag_drop)
...
c3 = tk.Checkbutton(window, text="Say opponent's moves.",
...
logs_text = tk.Text(text_frame, background="gray",
...
save_file = pl.Path("gui.bin")


def save_settings():
    with save_file.open("wb") as outfile:
        pickle.dump([field.get() for field in fields] + [token], outfile)


def load_settings():
    if save_file.exists():
        with save_file.open('rb') as infile:
